...
- **功能道具**：提示卡×10 / 复活卡×5 / 备考礼盒 pack 卡（图标方块 + 名称 + 描述 + 价格 + 购买）→ 点击购买后直接增加玩家库存，并提示「购买成功」；装饰/去广告仍为「即将上线」。
- **装饰藏品**：网格皮肤列表（预览 + 名称 + 状态），点击即可切换并立即作用于游戏网格；测试阶段不锁定未拥有皮肤，已拥有读 `ownedDecorations`。
- **去广告**尊享卡 → 即将上线 toast。
...
- 网格配色读取当前 `activeGridSkin` 主题（宣纸/竹简/龙纹/金箔/九五至尊）。
