...
void main() {
  ...
  final testIdioms = [
    ...
    Idiom(
      text: '画龙点睛',
      pinyin: 'hua4 long2 dian3 jing1',
      meaning: '比喻在关键处加上精辟语句',
    ),
    ...
    Idiom(
      text: '叶公好龙',
      pinyin: 'ye4 gong1 hao4 long2',
      meaning: '比喻表面上爱好实际上畏惧',
    ),
  ...
  if (inter != null) {
    ...
    print(
      '  ${a.idiom.text}第$ai个字"${a.idiom.text[ai]}" ↔ ${b.idiom.text}第$bi个字"${b.idiom.text[bi]}"',
    );
  } else {
    print('无交叉');
  }
    ...
    for (final placement in level.placements) {
      for (int k = 0; k < placement.idiom.text.length; k++) {
        ...
        if (cell.character != placement.idiom.text[k]) {
          print(
            '  ✗ 错误: ${placement.idiom.text}[$k] 应为"${placement.idiom.text[k]}"，实际为"${cell.character}"',
          );
          valid = false;
        }
      }
