...
const _agreementSections = <({String title, String body})>[
  ...
  (
    title: '三、使用规则',
    body:
        '您应合法、合理地使用本应用，不得利用本应用从事违法活动，不得干扰应用、广告服务、排行榜或其他服务的正常运行，也不得通过修改、作弊或自动化方式不当获取积分、道具、成就或排名。',
  ),
  (
    title: '四、广告与奖励',
    body:
  ...
  (
    title: '五、知识产权',
    body: '本应用的软件、界面、图形及其他原创内容受相关法律保护。成语、典故等传统文化资料可能来源于公共文化资料，仅用于学习与游戏体验。',
  ),
  (
    title: '六、服务变更与免责',
    body:
...
const _privacySections = <({String title, String body})>[
  ...
  (
    title: '二、广告服务',
    body:
        '本应用按 iOS 系统地区选择广告平台：中国大陆（CN）使用 Dirichlet 聚合，其他地区（包括港澳台）使用 Google AdMob。系统地区可由用户修改，不代表 GPS 或 IP 实际位置；修改后重启应用生效。Google 广告用于横幅、激励视频和插页式广告。Google 可能依据您的授权状态处理设备标识符、IP 地址、广告互动、诊断信息及大致位置等数据，用于广告投放、频次控制、反欺诈和效果衡量。iOS 会在需要时显示系统跟踪授权提示；拒绝跟踪不会阻止您使用游戏，但广告可能与您的兴趣无关。',
  ),
  (
    title: '三、Dirichlet 聚合广告',
    body:
        '国内横幅、插页广告和激励视频使用 Dirichlet 聚合 SDK（TapADN，上海艾得蒽数字科技有限公司及其关联方），并接入穿山甲（北京巨量引擎网络技术有限公司）适配器，由聚合平台按广告位配置选择广告来源。SDK 在您同意后自行采集设备基础信息、IDFV、IP 地址、网络状态、广告互动和诊断信息，用于广告投放、监测归因、反作弊和统计分析。IDFA 仅在 iOS 跟踪授权后访问；本应用不向广告 SDK 提供精确位置。拒绝国内广告授权不影响基本游戏功能，可通过本页“修改国内广告隐私授权”重新选择。第三方保存期限、权利行使和联系方式见各自完整隐私政策：Dirichlet https://ssp.dirichlet.cn/docs/agreement/；穿山甲 https://www.csjplatform.com/privacy/partner。',
  ),
  (
    title: '四、Game Center',
    body:
