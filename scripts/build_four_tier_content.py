...
def main():
    ...
    entries.sort()
    content = {'version': 2, 'legacyMaxId': 29502, 'inferenceBounds': [1, 5, 10],
               'entries': entries, 'additions': additions}
    evidence = {
        'version': 2, 'inferenceBounds': [1, 5, 10],
        'sourceHashes': {str(p.relative_to(ROOT)): hashlib.sha256(p.read_bytes()).hexdigest()
                         for p in [reviews / '有数据库ID_四字成语审核表.md',
                                   ...
                                   ROOT / 'data/scoring_progress.json']},
    ...
    report = ['# 四档分级导入与推断报告', '', '内容版本：2。教材/手工覆盖优先；审核状态不限制主线选词。', '',
