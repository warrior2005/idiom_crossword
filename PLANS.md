...
## 当前实现：四档词汇与全智能主线（2026-09-11）

[执行计划与验收标准](docs/specs/2026-09-11-four-tier-adaptive-mainline-execution-plan.md)已获批准并完成工程实现及本地验证，见[实现验收记录](docs/specs/four_tier_implementation_v2.md)。真机升级与真人试玩仍待进行。规则包括：教材与人工标注优先，整合旧465条，新增222条，剩余词快速推断四档；所有入库词参与生成，审核状态仅作记录；移除手动三档设置，自动适配只影响后续新题；干扰字按比例生成且至少4个，复用现有消歧。

涉及冲突规则，以该执行计划为准。下方2026-09-09清单保留为已完成首版的历史记录，新实现状态以本节和验收记录为准。

## 当前优先开发：成语分级与关卡体验改造（2026-09-09）

P0—P2 的首版代码已实现，结果见[实现与验收记录](docs/specs/mainline_implementation_v1.md)；词池人群校准和发布后的留存评估仍待完成。开发依据见 [成语分级与关卡体验改造方案](docs/specs/2026-09-09-difficulty-and-level-experience-redesign.md)。涉及难度与选词时，以新方案为准；下文的既有完成状态不代表本次改造已完成。

- [x] P0 工程首版：初始准入清单、5条拼音修正、最终配额与路径约束、安全回退、稳定ID、旧安装内容更新及旧题换题。
- [x] P1 工程首版：前20关题面、可变候选盘、科举等级解耦、主线生僻度边界和本地观察记录。
- [x] P2 工程首版：三种偏好、每5次终态的小幅适配、词级辅助／独立／延迟作答记录、到期词优先复习。
- [ ] 发布前：不同储备玩家试玩，校准初始词池与前200关参数；真实iOS设备升级、存档和交互验收。
- [ ] 发布后：用户级实验分流、留存及卡关基线对照，评估经验成长节奏。

配套迁移、测试及完成条件详见方案第 5—7 节。现有交叉布局、消歧、成长奖励与历史存档能力按方案保留。
...
| Database Schema（v9） | ✅ 完成 | 12 张表：进度/收藏/历史/装饰/存档/成就/设置；含跨关卡连胜字段 |
...
| 单元/集成测试 | ✅ 完成 | 95 个测试；真实代码覆盖率约 79%（不含生成代码） |
| 成长系统设计 | ✅ 完成 | 科举仕途 21 级（含 Lv.∞ 真龙天子），螺旋难度模型 |
...
| 游戏 UI | ✅ 完成 | 主界面/关卡选择/统计/成就/设置/学习/收藏搜索 |
...
| 学习模式 | ✅ 完成 | 通关后复习释义/出处/例句；自定义关卡已移除 |
...
| 2.3 | 提示系统 | 2.2 | 一字提示：每次消耗提示卡 | ✅ |
| 2.4 | 过关动画与音效 | 2.2 | 完成成语依次高亮、得分结算、关卡解锁动画 | ✅ |
| 2.5 | 关卡列表与选择 | 1.3 | 每页 24 关分页展示，显示完成状态 | ✅ |
| 2.6 | 科举仕途成长系统 | 2.1 | 实现设计文档 §2 等级体系（21级，指数经验曲线，称号+道具奖励） | ✅ |
| 2.7 | 螺旋难度关卡生成 | 2.1 | 实现设计文档 §4 螺旋难度模型（无限生成，8-12 条成语/关） | ✅ |
...
- 2026-07-31: 完成一字提示系统（×3/关 免费、之后消耗提示卡）
- 2026-07-31: 完成过关庆祝动画与"下一关"直通流程；重玩已通关关卡不再重复发放经验/收藏
- 2026-07-31: 新增关卡选择界面（分页网格，显示完成/当前/锁定状态，点击直接进入对应关卡）
...
- 2026-07-31: 完成学习模式（PRD 5.3 P3）：通关对话框新增"复习成语"，展示本关释义/出处/例句
...
- 2026-07-31: 新增完整通关流程端到端 widget 测试（候选字填字→过关对话框→经验/通关记录落库），螺旋混排测试改为固定种子消除随机性
...
- 2026-08-01: 修复 9 项反馈：弹框移除 emoji（问号来源）；每日挑战难度跟随当前关卡并上移一档、标题改"每日挑战"；统计页居中；成就扩展为 21 项分层里程碑（含计数）；XP 进度分母修正并用称号替代 Lv；关卡选择只展示已完成+当前关且格子显示数字；历史关卡冻结定义（schema v7，重玩同题）；完成成语释义加"释义："前缀
- 2026-08-01: 二轮反馈 6 项已修：关卡选择恢复分页（仅已完成+当前关分页）；主界面改显剩余经验"xxx经验 升至 称号"；收藏页每页 30 条分页；统计改 6 卡片每行 3 个且"最近通关"左对齐；游戏页等级显示"Lv.1 童生"；完成词条区固定高度避免布局抖动
- 2026-08-01: 按产品决策删除三项功能：成语提示按钮、长按空格看所属成语、全图揭示按钮；PRD §3.4/§6.3 与 PLANS 同步更新（提示系统收敛为"一字提示+提示卡"闭环）
